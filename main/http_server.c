...
/**
 * @brief handler to display list of available SSIDS
 * 
 */

static esp_err_t http_server_ssid_list_handler(httpd_req_t *req){
 ESP_ERROR_CHECK(esp_wifi_scan_start(NULL, true));
 ESP_LOGI(TAG,"/wifi SSID list called");
 //esp_wifi_scan_get_ap_records()
 uint16_t ap_count;
	uint16_t number_of_SSID = 5;
	wifi_ap_record_t ap_info_f[number_of_SSID];
    memset(ap_info_f, 0, sizeof(ap_info_f));
	 ESP_ERROR_CHECK(esp_wifi_scan_get_ap_records(&number_of_SSID, ap_info_f));
	          ESP_ERROR_CHECK(esp_wifi_scan_get_ap_num(&ap_count));
	             ESP_LOGI(TAG, "Total APs scanned = %u", ap_count);
					char json_String[100];
	         	sprintf(json_String,"{\"SSID\":[");
	         		              for (int i = 0; (i <5) && (i < ap_count); i++) {
	         		            	                ESP_LOGI(TAG, "SSID \t\t%s", ap_info_f[i].ssid);
	         		            	                char temp[50];
	         		            	                if(i>=5-1 || i>=ap_count-1){
	         		            	                	sprintf(temp, "\"%s\"",ap_info_f[i].ssid);
	         		            	                }
	         		            	                else
	         		            	            {
	         											sprintf(temp,"\"%s\",",ap_info_f[i].ssid);}
...
/**
 * @brief mock uri handler to proceeed things
 * 
 */
esp_err_t mock_uri_handler(httpd_req_t *req){

	ESP_LOGI(TAG,"URI CALLED PLAZ WORK");
	return ESP_OK;
}
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		//call uri for index
		/*httpd_uri_t call_index={
			.uri = "/redirect",
			.method = HTTP_GET,
			.handler = mock_uri_handler,
		};
		httpd_register_uri_handler(http_server_handle, &call_index);*/
		...
		httpd_uri_t SSID_LIST = {
		.uri = "/ssid.json",
		.method = HTTP_GET,
		.handler = http_server_ssid_list_handler,
		.user_ctx= NULL
	};
	httpd_register_uri_handler(http_server_handle,&SSID_LIST);
		

	/*httpd_uri_t mock_uri={
		.uri = "/",
		.method = HTTP_POST,
		.handler = mock_uri_handler,
	};
	httpd_register_uri_handler(http_server_handle,&mock_uri);*/
